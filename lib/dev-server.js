...
module.exports.setupDevServer = ({ server, templatePath, onUpdate }) => new Promise((resolve, reject) => {
  ...
  clientCompiler.hooks.done.tap('cli ssr', async stats => {
    ...
    const clientSsrPlugin = (clientConfig.plugins.find((p) => (p.__pluginName || p.constructor.name) === 'ssr') || {}).options || {};
    clientManifest = JSON.parse(readFile(
      devMiddleware.fileSystem,
      clientSsrPlugin.filename || 'vue-ssr-client-manifest.json'
    ))
  ...
  serverCompiler.watch({}, (err, stats) => {
    ...
    serverBundle = JSON.parse(readFile(serverMfs, ssrPlugin.filename || 'vue-ssr-server-bundle.json'))
    update()
    onCompilationCompleted()
  })
